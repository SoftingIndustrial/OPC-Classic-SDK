...
public:
		...
		AddressSpaceElement*& anElement)
	{
		...
		static tstring path = g_stringBasedNodeName;
		...
		else if ((anElementID.find(g_stringBasedNodeName) == 0) && (anElementID.find(g_stringBasedItemName) != tstring::npos))
		{
		...
		if (g_stringBasedNodeName.length() > 0)
		{
			if (anElementId.compare(g_stringBasedNodeName) == 0)
			{
				...
				for (unsigned int i = 0; i < g_stringBasedItemCount; ++i)
				{
					...
					_stprintf_s(buffer, l_format, g_stringBasedNodeName.c_str(), g_stringBasedItemName, i);
