...
public:
	...
	{
		m_cellDefault.SetTextClr(clr);
	}
	COLORREF GetTextColor()
	{
		return m_cellDefault.GetTextClr();
	}
	void     SetTextBkColor(COLORREF clr)
	{
		m_cellDefault.SetBackClr(clr);
	}
	COLORREF GetTextBkColor()
	{
		return m_cellDefault.GetBackClr();
	}
	...
	{
		return m_cellFixedRowDef.GetTextClr();
	}
	...
	{
		return m_cellFixedRowDef.GetBackClr();
	}
	...
	{
		return m_cellDefault.GetMargin();
	}

	int      GetDefCellHeight() const
	{
		return m_cellDefault.GetHeight();
	}
	...
	{
		return (int)m_SelectedCellMap.GetCount();
	}
	...
	{
		return m_bSingleRowSelection & m_bListMode;
	}
	...
	{
		m_bFixedColumnSelection = bSelect;
	}
	BOOL GetFixedColumnSelection()
	{
		return m_bFixedColumnSelection;
	}
	void SetFixedRowSelection(BOOL bSelect)
	{
		m_bFixedRowSelection = bSelect;
	}
	BOOL GetFixedRowSelection()
	{
		return m_bFixedRowSelection;
	}
...
public:
	...
	{
		m_bWysiwygPrinting = bEnable;
	}
	BOOL GetWysiwygPrinting()
	{
		return m_bWysiwygPrinting;
	}

	void SetShadedPrintOut(BOOL bEnable = TRUE)
	{
		m_bShadedPrintOut = bEnable;
	}
	BOOL GetShadedPrintOut(void)
	{
		return m_bShadedPrintOut;
	}
...
protected:
	...
	{
		return ((m_nBarState & GVL_VERT) > 0);
	}
	BOOL IsVisibleHScroll()
	{
		return ((m_nBarState & GVL_HORZ) > 0);
	}
...
inline CGridCellBase* CGridCtrl::GetDefaultCell(BOOL bFixedRow, BOOL bFixedCol) const
{
	...
	{
		return (CGridCellBase*) &m_cellFixedRowColDef;
	}

	if (bFixedRow)
	{
		return (CGridCellBase*) &m_cellFixedRowDef;
	}

	if (bFixedCol)
	{
		return (CGridCellBase*) &m_cellFixedColDef;
	}
...
inline CGridCellBase* CGridCtrl::GetCell(int nRow, int nCol) const
{
	...
	if (GetVirtualMode())
	{
		...
		{
			gvdi.item.nState |= GVIS_SELECTED;
		}

		if (nRow < GetFixedRowCount())
		{
			gvdi.item.nState |= (GVIS_FIXED | GVIS_FIXEDROW);
		}

		if (nCol < GetFixedColumnCount())
		{
			gvdi.item.nState |= (GVIS_FIXED | GVIS_FIXEDCOL);
		}

		if (GetFocusCell() == CCellID(nRow, nCol))
		{
			gvdi.item.nState |= GVIS_FOCUSED;
		}
